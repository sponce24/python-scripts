...
#  fan_addQ = N.einsum('ijklmno,plnkm->ijop',FAN,displ_red_FAN2) 
  ...
  if type == 1:
    ...
    fan_add = N.einsum('ijk,kij->ij',fan_addQ[:,:,:,imode],div1[imode,:,:,:]+div2[imode,:,:,:,]) # ikpt,iband,jband,imod

    print "Now compute generalized g2F Eliashberg electron-phonon spectral function ..." 
 
    fan_tmp = fan_addQ[:,:,imode] # (ikpt, iband, imode)
    ddw_tmp = ddw_addQ[:,:,imode] # (ikpt, iband, imode)
...
# Einstein sum make the vector matrix multiplication ont the correct indices
  ...
  fan_corr = N.einsum('jk,l->ljk',fan_corrQ[imode,:,:],2*bose[imode,:]+1.0)
  ddw_corr = N.einsum('jk,l->ljk',ddw_corrQ[imode,:,:],2*bose[imode,:]+1.0)
...
# Now compute active space
  ...
  if type == 1: 
    ...
    tmp = N.einsum('ijk,m->mijk',ddw_addQ[:,:,:,imode],2*bose[imode,:]+1.0) # tmp,ikpt,iband,jband
    ...
    fan_add = N.einsum('ijkl,lmkij->mij',fan_addQ[:,:,:,imode],div1[imode,:,:,:,:]+div2[imode,:,:,:,:]) # ikpt,iband,jband,imode
